...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    if (trip.createdBy.toString() !== req.user.user_id && !trip.participants.includes(req.user.user_id)) {
    ...
    if (trip.endDate) {
      ...
      if (end < today) {
        return res.status(400).json({ msg: 'Cannot add expense to a past trip' });
      }
    }
