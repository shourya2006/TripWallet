...
router.get('/:id', auth, async (req, res) => {
  try {
    const trip = await Trip.findById(req.params.id).populate('participants', 'username email');
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    if (existingTrip) {
      return res.status(400).json({ msg: 'You already have a trip with this name.' });
    }

    let tripImage;
    try {
      const unsplashResponse = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(title)}&orientation=landscape&per_page=1&client_id=acngLKDQjaIDcf_DPzuCvxzox_uusRJCI3ylzXX01B8`);
...
router.post('/:id/leave', auth, async (req, res) => {
  try {
    ...
    if (trip.createdBy.toString() === req.user.user_id) {
      return res.status(400).json({ msg: 'Creator cannot leave the trip. Delete it instead.' });
    }

    const isParticipant = trip.participants.some(p => p.toString() === req.user.user_id);
    if (!isParticipant) {
      return res.status(400).json({ msg: 'User is not a participant' });
    }

    const initialCount = trip.participants.length;
    trip.participants = trip.participants.filter(p => p.toString() !== req.user.user_id);
    await trip.save();

    console.log(`User ${req.user.user_id} left trip ${trip._id}. Participants: ${initialCount} -> ${trip.participants.length}`);
...
router.post('/:id/participants', auth, async (req, res) => {
  ...
  try {
    ...
    if (!isAuthorized) {
      return res.status(403).json({ msg: 'Not authorized to add participants to this trip' });
    }
